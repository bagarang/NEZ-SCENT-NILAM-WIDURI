 ...
 */
var API_URL = 'https://script.google.com/macros/s/AKfycbzzfRfjX9RGT-OJveZy_NXk1OKkagUAdKp8NJWZ5oMH3VFBEOEjNaSuNJnsDIn43gSmzA/exec';
...
function renderDashboard(dateFrom, dateTo) {
  api('apiGetDashboard', STATE.token, dateFrom || null, dateTo || null).then(function (d) {
    var c = document.getElementById('content');
    c.innerHTML =
      '<div class="page-header">' +
        '<h2>Dashboard</h2>' +
        '<div class="toolbar">' +
          '<input type="date" id="dashFrom" class="search-input" style="min-width:150px;" value="' + esc(dateFrom || '') + '">' +
    ...
    document.getElementById('dashApplyBtn').onclick = function () {
      renderDashboard(document.getElementById('dashFrom').value, document.getElementById('dashTo').value);
    };
...
var ENTITY_ACTIVITY = {
  ...
  columns: [
    ...
    { field: 'linkScreenshot', label: 'Screenshot', link: true },
...
function crudRowHtml_(entity, r) {
  var cells = entity.columns.map(function (col) {
    ...
    if (col.fmt === 'date') v = fmtDate(v);
      ...
      v = v ? '<a href="' + esc(v) + '" target="_blank" rel="noopener">\u{1F517} Lihat</a>' : '<span style="color:var(--text-muted);">-</span>';
    } else if (col.wrap) {
      // Tampilkan teks lengkap (tidak dipotong "...") tapi tetap rapi:
      // wrap ke baris baru di dalam kolom, bukan melebar/terpotong.
      tdStyle = ' style="max-width:280px;white-space:normal;word-break:break-word;"';
      v = esc(v);
    } else if (col.truncate) v = esc(truncate_(v, 50));
    else if (col.daysUntil) v = daysUntilHtml_(v);
    else v = esc(v);
    return '<td' + tdStyle + '>' + v + '</td>';
  }).join('');
...
function renderLookup() {
  ...
  c.innerHTML =
    ...
    '<div class="card" style="margin-bottom:16px;max-width:480px;">' +
      '<div class="field"><label>Cari PIC Agen (Contact Person)</label>' +
        searchComboHtml_('lookupSelect', 'lookupSelect_search', 'lookupSelect_dd', 'Ketik nama / kode agen untuk mencari...', '', '') +
      '</div>' +
    '</div>' +
    '<div id="lookupResult"></div>';

  wireSearchCombo_('lookupSelect', 'lookupSelect_search', 'lookupSelect_dd', function () { return opts; }, function (val) {
...
/** Tabel mini di Lookup, kolomnya persis sama dengan definisi kolom tab aslinya. */
function miniTableHtml_(columns, rows) {
  var thead = '<tr>' + columns.map(function (col) { return '<th>' + esc(col.label) + '</th>'; }).join('') + '</tr>';
  var tbody = rows.length
    ? rows.map(function (r) {
        return '<tr>' + columns.map(function (col) {
          ...
          else if (col.truncate) v = esc(truncate_(v, 40));
          else if (col.daysUntil) v = daysUntilHtml_(v);
          else v = esc(v);
          return '<td>' + v + '</td>';
        }).join('') + '</tr>';
      }).join('')
    : '<tr><td colspan="' + columns.length + '" class="empty-state">Belum ada data.</td></tr>';
  return '<table class="data-table"><thead>' + thead + '</thead><tbody>' + tbody + '</tbody></table>';
}

// Kolom Agen sengaja tidak diikutkan di sini karena konteksnya sudah spesifik satu agen.
var LOOKUP_ACTIVITY_COLS = [
  { field: 'date', label: 'Tanggal', fmt: 'date' }, { field: 'contactMethod', label: 'Metode' },
  { field: 'admin', label: 'Admin' }, { field: 'notes', label: 'Notes', truncate: true }
];
var LOOKUP_PI_COLS = [
  { field: 'nomorPI', label: 'No. PI' }, { field: 'dealValue', label: 'Nominal', fmt: 'money' },
  { field: 'piDate', label: 'Tanggal', fmt: 'date' }, { field: 'statusPI', label: 'Status PI', badge: true },
  { field: 'notes', label: 'Notes', truncate: true }
];
var LOOKUP_SO_COLS = [
  { field: 'soNumber', label: 'No. SO' }, { field: 'amount', label: 'Nominal', fmt: 'money' },
  { field: 'dueDate', label: 'Due Date', fmt: 'date' }, { field: 'daysUntilDue', label: 'Sisa Hari', daysUntil: true },
  { field: 'displayStatus', label: 'Status', badge: true }, { field: 'notes', label: 'Notes', truncate: true }
];
var LOOKUP_GIFT_COLS = [
  { field: 'deliveryDate', label: 'Tanggal', fmt: 'date' }, { field: 'description', label: 'Deskripsi' },
  { field: 'value', label: 'Value', fmt: 'money' }, { field: 'salesRep', label: 'Pengirim' }
];
...
function paintLookupResult(res) {
  ...
  document.getElementById('lookupResult').innerHTML =
    '<div class="lookup-grid">' +
      ...
      '<div class="card" style="overflow-x:auto;"><h3>Recent Activity</h3>' + miniTableHtml_(LOOKUP_ACTIVITY_COLS, res.activity) + '</div>' +
      '<div class="card" style="overflow-x:auto;"><h3>Recent Proforma Invoice</h3>' + miniTableHtml_(LOOKUP_PI_COLS, res.pi) + '</div>' +
      '<div class="card" style="overflow-x:auto;"><h3>Recent Sales Order</h3>' + miniTableHtml_(LOOKUP_SO_COLS, res.so) + '</div>' +
      '<div class="card full" style="grid-column:1/-1;overflow-x:auto;"><h3>Gifts</h3>' + miniTableHtml_(LOOKUP_GIFT_COLS, res.gifts) + '</div>' +
    '</div>';
}
...
function renderSetup() {
  api('apiGetSetupLists', STATE.token).then(function (lists) {
    ...
    c.innerHTML =
      ...
      '</div>' +
      '<div class="card" style="max-width:480px;margin-bottom:16px;">' +
        '<div class="field"><label>Target Kontak per Bulan</label>' +
          '<input id="targetKontakInput" type="number" min="0" value="' + esc(lists.targetKontak || 0) + '">' +
          '<div style="font-size:12px;color:var(--text-muted);margin-top:4px;">Dipakai untuk hitung "% Pencapaian Kontak" di Dashboard.</div>' +
        '</div>' +
        '<button class="btn" id="saveTargetBtn">Simpan Target</button>' +
      '</div>' +
    ...
    document.getElementById('saveTargetBtn').onclick = function () {
      api('apiSaveTargetKontak', STATE.token, document.getElementById('targetKontakInput').value)
        .then(function () { toast('Target tersimpan.', 'success'); })
        .catch(function (err) { toast(err.message, 'error'); });
    };
